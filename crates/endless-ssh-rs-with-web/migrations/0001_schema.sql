...
-- A row is one completed connection, inserted at disconnect. Buckets and retention key on disconnected_at, so an insert always lands at "now", inside every refresh window.
CREATE TABLE
    connections (
        id BIGINT GENERATED ALWAYS AS IDENTITY,
        connected_at TIMESTAMPTZ NOT NULL,
        disconnected_at TIMESTAMPTZ NOT NULL,
        -- measured by the client on a monotonic clock, not the difference of the two timestamps
        ...
        -- a country is a code and a name
...
-- for the replay cursor (id > $1) and the totals watermark (MAX(id))
CREATE INDEX ON connections (id);

-- All-time counters. Raw retention is 24 hours, so the rollups cannot recompute these. Maintained in the insert transaction instead.
CREATE TABLE
    totals (
...
-- Rollup chain, finest to coarsest. A continuous aggregate cannot refresh inside a transaction, hence WITH NO DATA.
...
-- long ranges read 1h and 1day, so both serve their un-materialized head live
...
-- connections_1day is kept forever
