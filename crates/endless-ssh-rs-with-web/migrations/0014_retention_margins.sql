-- Retention must exceed the downstream aggregate's start_offset, the margin is what
-- allows re-refreshing after downtime or a stalled job before the source rows are gone.
SELECT
