...
describe("topCountries", () => {
    it("sums a country across buckets", () => {
        ...
        expect(totals).toEqual([
            { country: { code: "US", name: "United States" }, connects: 5, bytes_sent: 150, time_spent: 20 },
        ]);
    });
    ...
    it("sorts by connects descending and truncates to the limit", () => {
        const totals = topCountries(
            [
                ...
                row("2026-01-01T00:01:00Z", { country: { code: "DE", name: "Germany" }, connects: 3 }),
                row("2026-01-01T00:01:00Z", { country: { code: "FR", name: "France" }, connects: 2 }),
            ],
            2,
        );
    ...
    it("groups rows without a country under one entry", () => {
        const totals = topCountries(
            [
                ...
                row("2026-01-01T00:02:00Z", { country: null, connects: 1 }),
            ],
            5,
        );

        expect(totals).toHaveLength(1);
        expect(totals[0]).toMatchObject({ country: null, connects: 2 });
    });
});
